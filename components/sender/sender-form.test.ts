...
const serviceArea = "Service area: Kuala Lumpur, Petaling, Hulu Langat, Sepang";
...
describe("Sender registered declaration", () => {
  it("limits the statement to the amount condition and reads thresholds from policy", () => {
    const view = render(createElement(SenderForm, { addresses, serviceArea, policy: { ...policy, cosignOverSen: 35000 } }));
  ...
  it("does not invent a zero threshold when no amount condition exists", () => {
    const view = render(createElement(SenderForm, { addresses, serviceArea, policy: { ...policy, cosignOverSen: null } }));
  ...
  it("explains why batch scanning cannot create a single shipment", () => {
    const view = render(createElement(SenderForm, { addresses, policy, serviceArea }));
