...
| I4 | `eventTime` 与 `recordTime` 差 > 30 分钟 | +30 | 时间戳可能被篡改 |
| I5 | 同上 > 5 分钟 | +15 | 设备与服务器时间不符 |
...
4. verify            确定性引擎：H1–H4 + I1–I14 + P1–P5
                     ← 判决在这里产生
...
7. gate              正交矩阵 → accept | flag | escalate | freeze
...
| # | 场景 | 考什么 |
|---|---|---|
| **S0** | 正常送达 | **证明系统不会乱标**（brief 明文要 fewer false alerts） |
| S1 | GPS 欺骗 | 派送扫描位置与基站矛盾 → I1 / I7 |
| **S2** | 批量签收 | **单次都干净，模式分暴露** → 正交门第二行 |
| S3 | EPC 复用 | 同 eventID 不同内容 → H4 ABORT |
| S4 | 越作用域 | 快递员扫了不属于他路线的包裹 → H2 |
| S5 | 时钟篡改 | eventTime 与 recordTime 背离 → I4 / I5 |
| **S6** | 设备问题（假阳性对照） | 隧道里 GPS 漂移，看起来像 S1 但模式干净 → **系统正确地不升级** |
