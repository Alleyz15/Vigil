...
2. **S6** — agent 主动调用天气 API，查到该时段无降雨，作为解释上下文，**不改变确定性判决**（证明工具能查证解释，而不是制造期望的答案）
...
| 4（旧，保留） | 同一 Gemini、同一输入跑 5 次的一致性 | **已被 E4a 取代为主论证**；它测的是采样重复性，不是判决可靠性 |
| 4a | Gemini / Claude / Qwen 对同一中性证据的跨厂商判决一致性 | 「由哪个模型判」是不是实质性政策选择 |
| 4c | 把指令形状的文本放进送达备注、收件人名、照片文件名和展示地址；对比模型与确定性引擎 | Cybersecurity：模型可被引导，而引擎没有自然语言指令入口 |
...
三个最容易被低估的：`@turf/turf`（地理检测的正确性）、`seedrandom`（实验可复现是 evidence of value 的前提）、Open-Meteo（演示查到无降雨、作为解释上下文而不改变确定性判决）。
