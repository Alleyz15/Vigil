...
### 1j. Attach an architectural claim to the component, never to the page

`AxisPair` renders the two scores, and the heading **"Two axes, never summed"** lives inside it
rather than on the page that uses it.

That is not a layout preference. Two small adjacent numbers are read as components of a total —
which is exactly the reading rule 2 exists to prevent — so the heading is part of what makes the
component correct, not decoration around it. Put it on the page and the next page to show those
scores is one nobody remembered to caption.

> **A claim attached to a component travels with every use of it. A claim written on a page
> depends on the next page remembering.**

This is the same logic as the nine mechanically enforced guards, one layer up: **what is
structural does not erode, and what is merely decided does.** Session 12 decided a type size and
it eroded across twelve files in six sessions; the guards have never regressed once. A heading
that must be re-typed per page is a decision. A heading the component carries is structure.

**So: when a view states something the architecture depends on** — the axes are separate, this
score was not evaluated, this credential is a sidecar, nothing was sealed — **put the words where
...
The project has **two kinds of page**, and a convention written for one is wrong for the other.
Session 19 added the second kind, so the distinction is now stated rather than assumed.

| | The console | The narrative |
|---|---|---|
| Routes | `/operator/*`, `/demo/*`, `/verify` | `/` |
| Read by | one operator, dozens of times a shift | a judge, once, for ninety seconds |
| Costs | latency on repeated work | failing to land the argument at all |
| Motion | banned on the gate drag and the SSE sequence; `motion` only, narrowly | scroll-driven narrative is the point |
| Libraries | no `lenis`, no `mermaid` | both, `mermaid` by dynamic import only |
| Density | dense is fine, cramped is not | generous; one idea per section |

**The console's animation bans are not aesthetic purity, they are about a person doing work.**
Easing on the gate drag reads as lag; entrance animation on the SSE sequence makes it impossible
to tell which delays are computation and which are decoration, which is the entire point of
showing the reasoning. **None of those reasons apply to a page somebody scrolls once.**

Equally, the narrative's freedoms do not travel back. `lib/purity.test.ts` fails if a console tree
imports `lenis` or `mermaid` — **the eighth constraint** — because inertial scroll and diagram
rendering on a work queue add latency to work that is repeated, for a viewer who is not there.
