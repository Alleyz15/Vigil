...
[DATASET.md](./DATASET.md). Deterministic experiments can be recomputed from fixed seeds,
inputs and the corresponding code version. Historical online model measurements refer to
archived responses and exact model IDs, not a promise that another live call will return
the same bytes. The Session 13 E4/E5 responses are archived in `9fb0d19`; the Session 14
E4a/E4c and cross-provider E5 responses are archived in `7a7c1d7`.
...
leg 8 by **P2 alone**. The modelled forty-event batch has a single-event total score of zero
per event. That is not a claim that every check returned `clear`: a `not_evaluated` check
also contributes no points. The summary CSV records detection outcomes, not a complete
per-event, per-check trace.
...
**In this sample, only Qwen had a `flag` -> `accept` pair.** E4a separately observed Qwen
accepting the obvious S1 spoof 5/5 while returning no reasoning at all. Four single paired
calls per model cannot rank susceptibility to manipulation or establish that model size
caused the transition. This observed boundary crossing is a qualification on the
local-inference path, not a comparative steerability ranking — see Known Limitations.
...
**E6's incorrect prediction is retained here rather than edited:** its edge was predicted
to move under noise and did not. Why it did not is the more useful result — see
*The prediction was wrong, and the reason is the finding*.
